...
/**
 * A ClumpMethod designed for ICS-209 data.
 */
@MetaInfServices
public class ICS209ClumpMethod implements ClumpMethod {
    private static final double ONE_ACRE_CIRCLE_RADIUS_METERS = 35.890868;
    private static final Logger log = LoggerFactory.getLogger(ICS209ClumpMethod.class);
    private final GeometryBuilder geometryBuilder;
    private final Source source;

    public ICS209ClumpMethod(GeometryBuilder geometryBuilder, Source source) {
    ...
    public Collection<Clump> clump(Iterable<RawData> rawData) {
        ...
        for(RawData rec : rawData) {
            String incidentNumber = rec.get("incident number");
            String incidentType = rec.get("incident type");
            if(incidentType == null || !(incidentType.equals("WF") || incidentType.equals("WFU") || incidentType.equals("RX"))) {
                log.info("Ignoring ICS-209 report \"{}\" because it is not a fire (incident type: \"{}\")",
                        incidentNumber, incidentType);
                continue;
            }
            ...
            // and calculate a circle of that area centered on the ICS209
