...
/**
 * An AssociationMethod designed for FETS data.  Will work for data sets with multiple records 
 * with different dates and daily area burned for each record (as opposed to ICS-209, which
 * has cumulative area burned).
 */
@MetaInfServices
public class FETSAssociationMethod implements AssociationMethod {
    private static final Logger log = LoggerFactory.getLogger(FETSAssociationMethod.class);
    private final int numForwardDays;
    private final int numBackwardDays;
    private final String associationField;
    
    public FETSAssociationMethod(
            ...
            @Attribute(name="associationField",
                    description="The name of the field of the report that should be used to associate records")
    ...
    public void associate(Clump clump, QueryableFireSet fireSet) {
        ...
        RawData theReport = rawData.get(0);
        
        String associationFieldValue = theReport.get(associationField);
        
        if(associationFieldValue == null) {
            log.warn("Unable to find a value for field \"{}\" on RawData #{}; ignoring",
                    associationField, theReport.getId());
            return;
        }
        ...
        // Calculate total area as the sum of all clump areas
        Set<Clump> clumps = result.getClumps();
        ...
        // Merge data attributes into Fire attributes
        for(Map.Entry<String, String> entry : theReport.entrySet()) {
            if(!result.containsKey(entry.getKey())) {
                result.put(entry.getKey(), entry.getValue());
            } else if(theReport.getEndDateTime().isAfter(previousEndDate)) {
                result.put(entry.getKey(), entry.getValue());
            }
        }
    }
